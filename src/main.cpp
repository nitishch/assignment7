...
//  glutMainLoop();
  float32 s=0.0f, coll=0.0f, velo=0.0f, posi=0.0f;
  struct timeval a;
  struct timezone b;
  settimeofday(&a,&b);
  gettimeofday(&a,&b);  
  double time=a.tv_sec+(a.tv_usec/1000000.0);
  int v = atoi(argv[1]);
  for(int i=0;i<v;i++){
	(*test).step(&settings);
	s += (*((*test).get_world())).GetProfile().step;
	coll += (*((*test).get_world())).GetProfile().collide;
	velo += (*((*test).get_world())).GetProfile().solveVelocity;
	posi += (*((*test).get_world())).GetProfile().solvePosition;
  }
  gettimeofday(&a,&b);
  time = a.tv_sec+(a.tv_usec/1000000.0) - time ;
